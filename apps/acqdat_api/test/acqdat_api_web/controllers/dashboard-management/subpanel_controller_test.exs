defmodule AcqdatApiWeb.DashboardManagement.SubpanelControllerTest do
  ...
  describe "create/2" do
    ...
    test "Return error when duplicate name is used during create subpanel", %{conn: conn} do
      ...
      post(conn, Routes.subpanel_path(conn, :create, org_id, dashboard_id, panel.id), params)
  ...
  describe "index/2" do
    ...
    test "Returns empty list when panel has no subpanel", %{conn: conn, org: org} do
      subpanel = insert(:subpanel)
      ...
      assert assert length(resp["subpanels"]) == 0
    end
  end
