defmodule AcqdatCore.Model.EntityManagement.Project do
  ...
  def return_count(%{"type" => "ProjectArchived", "org_id" => org_id}) do
    query =
      from(p in Project,
        where: p.archived == true and p.org_id == ^org_id,
        select: count(p.id)
      )
  ...
  def return_count(%{"type" => "Project", "org_id" => org_id}) do
    query =
      from(p in Project,
  ...
  def return_count(%{"type" => "Project"}) do
    query =
      from(p in Project,
        select: count(p.id)
      )
  ...
  defp group_by_senor_type(entity) do
    ...
    Enum.reduce(grouped_data, [], fn {key, _val}, acc ->
